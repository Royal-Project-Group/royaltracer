...
void IntersectTri(struct Ray* ray, __global struct Triangle* tri, uint instPrim, bool drawBackfaces )
{

    if(dot(tri->vn, -ray->dir) < 0.0f && !drawBackfaces){
        return;
    }
    ...
    float u = f * dot( s, h );
    if (u < 0-EPSILON || u > 1+EPSILON) return;
    const float3 q = cross( s, edge1 );
    const float v = f * dot( ray->dir, q );
    if (v < 0-EPSILON || u + v > 1+EPSILON) return;
    const float t = f * dot( edge2, q );
    if (t > EPSILON && t < ray->hit.t-EPSILON)
...
float IntersectAABB(struct Ray* ray, __global struct BVHNode* node) {
    ...
    tmin = max(tmin - EPSILON, min(ty1, ty2));  // Adjusted for EPSILON
    tmax = min(tmax + EPSILON, max(ty1, ty2));  // Adjusted for EPSILON
    float tz1 = (node->min.z - ray->orig.z) * ray->rD.z, tz2 = (node->max.z - ray->orig.z) * ray->rD.z;
    tmin = max(tmin, min(tz1, tz2));           // Adjusted for EPSILON
    tmax = min(tmax, max(tz1, tz2));           // Adjusted for EPSILON
    if (tmax >= tmin && tmin < ray->hit.t && tmax > 0) return tmin; else return 1e30f;
}

void BVHIntersect(struct Ray* ray, uint instanceIdx,
                  __global struct Triangle * tri, __global struct BVHNode* bvhNode, __global uint* triIdx, bool drawBackfaces, int tri_Cnt)
{
    //printf("START TRAVERSAL: %d\n", bvhNode[6].triCount);
    ...
    while (1)
    {
        if (node->triCount > 0) // isLeaf()
        {
            for (uint i = 0; i < node->triCount; i++)
            {
                ...
                IntersectTri( ray, &tri[instPrim & 0xfffff], instPrim ,drawBackfaces);
                                    //printf("v0 Dir x: %f\n", ray->hit.t);
            }
...
float generateRandom(uint *seed0, uint *seed1) {
    ...
    combined_seed = combined_seed * 6364136223846793005UL;
    combined_seed += 1442695040888963407UL;
...
/*bool intersectTri(
    struct Ray* ray,
    const __global struct Triangle* tri,
    float* t,
    bool recognizeBackfaces
)
{
...
float3 sampleSimpleLights(
    ...
    for(int i=0;i<poiCount;i++){
                ...
                BVHIntersect(&ray, 0, triangles, nodes,triIdx, false, triCount);
                int hitIDTri = -1;
                if(ray.hit.t < 1e30){
                    hitDistTri = ray.hit.t;
                    hitIDTri = ray.hit.instPrim & 0xfffff;
                    //printf("%d\n", hitIDTri);
                }
                ...
                if(hitDistSph + EPSILON > lDir && hitDistTri + EPSILON > lDir){
                    ...
                    weightedIntensity += (angleFactor * distanceFactor) * pointLights[i].Ke * (1.0f - pdf);
                }
    }
...
float3 getUVOfHitPandTri(struct Triangle hitTriangle, float3 P){
...
float3 tracePath(
        ...
        __global Map* textures,
        __global float3* texturedata,
        __global Map* bumpMaps,
        __global float* bumpMapData,
        __global Map* normalMaps,
    ...
    )
{
    ...
    for (int j = 0; j < bounces; j++){
        ...
        BVHIntersect(&ray, 0, triangles, nodes,triIdx, isRefracting, triCount);
        int hitIDTri = -1;
        if(ray.hit.t < 1e30){
            hitDistTri = ray.hit.t;
            hitIDTri = ray.hit.instPrim;
            //(hitIDTri!=0)?printf("CurrentObj: %d\n", hitIDTri):printf("");
        }
        ...
        if(hitType==-1){
            accum_color += mask * (float3)(20.0f, 20.0f, 20.0f);  // Here mask will affect the color added
            return accum_color;
        }
        else if(hitType==0){
            ...
            struct Triangle hitTriangle = triangles[currentObj];
            normal = normalize(hitTriangle.vn);
            material = materials[hitTriangle.materialID];

            float3 bary = getUVOfHitPandTri(hitTriangle, hitPos);

            float2 t0 = hitTriangle.uv1;
            float2 t1 = hitTriangle.uv2;
            float2 t2 = hitTriangle.uv3;
           ...
           /*float2 t0 = {0.0f, 0.0f};
            ...
            pTex = t0 + bary.y*(t1-t0) + bary.z*(t2-t0);
        }
        float3 colorVector;
        float3 dir_temp = dir;
        ...
        if(hitType==1 && material.normalMap_id != -1){
                struct Triangle hitTriangle = triangles[currentObj];
                Material mat = materials[hitTriangle.materialID];
                int normalWidth = normalMaps[mat.normalMap_id].width;
                int normalHeight = normalMaps[mat.normalMap_id].height;
                int offset = normalMaps[mat.normalMap_id].offset;

                int xN = (int)(pTex.x * normalWidth);
                int yN = (int)(pTex.y * normalHeight);

                int index = (xN + yN * normalWidth) % (normalWidth*normalHeight);

                float3 normalMapVector = normalMapData[index + offset];
                normalMapVector *= 2.0f;
                normalMapVector -= (float3)(1.0f, 1.0f, 1.0f);
                ...
                //if(normal.z > 0.5f || normal.z < -0.5f)
                 //   printf("soos");
               //printf("normal: %f, %f, %f \n + normalMapNormal: %f, %f, %f \n = rotatedVector: %f, %f, %f\n \n \n \n", normal.x, normal.y, normal.z, normalMapVector.x, normalMapVector.y, normalMapVector.z, rotatedVector.x, rotatedVector.y, rotatedVector.z);

                normal = normalize(rotatedVector);
        }

        if(hitType == 1 && material.bumpMap_id != -1){
            int bumWidth = bumpMaps[material.bumpMap_id].width;
            int bumHeight = bumpMaps[material.bumpMap_id].height;
            int offset = bumpMaps[material.bumpMap_id].offset;

            float stepWidth = 1.0f / bumWidth;
            float stepHeight = 1.0f / bumHeight;

            int xN = (int)(pTex.x * bumWidth);
            int yN = (int)(pTex.y * bumHeight);
            ...
            float heightCenter = bumpMapData[index + offset];
            float heightRight = bumpMapData[indexRight + offset];
            float heightUp = bumpMapData[indexUp + offset];
            ...
            normal += (dU * tangent + dV * bitangent) * bumpIntensity;
        ...
        if (randomScatter < fogScatterIntensity && !isRefracting) {
            float pdf = 0.0f;
            // Update the ray's origin based on scattering
            orig = orig + dir * (t-(EPSILON*2.0f)) * (randomScatter/(fogScatterIntensity+EPSILON*4.0f));

            //Do the light sampling from the new origin
             accum_color += (sampleSimpleLights(triangles, spheres, pointLights,material,(float3)(0.0f,0.0f,0.0f),nodes, triIdx, triCount,sphCount,  poiCount, orig, pdf)) * mask;
            ...
            if (!temp_isRefracting && isRefracting) {
                ...
                if(subsurfaceIteration>1){
                    //We did some subsurface steps, so notify the sampleLight to use 1.0f as pdf
                    pdf = 0.0f;
        ...
        if(hitType==1 && material.texture_id != -1){
                struct Triangle hitTriangle = triangles[currentObj];
                Material mat = materials[hitTriangle.materialID];
                ...
                int index = (xN + yN * textureWidth )% (textureWidth*textureHeight);

                // printf("Width: %d, Height: %d, Offset: %d, xN: %d, yN: %d\n", textureWidth, textureHeight, offset, xN, yN);



                float3 col = texturedata[index + offset];
            ...
            accum_color += (material.Ke + sampleSimpleLights(triangles, spheres, pointLights,material,-normal,nodes, triIdx, triCount,sphCount,  poiCount, hitPos, pdf)) * mask;
       ...
       )
{
    ...
    for (int i = 0; i < samples; i++) {
        ...
        float offsetX = (generateRandom(&seed0, &seed1) - 0.5f) * pixelSize;
        float offsetY = (generateRandom(&seed0, &seed1) - 0.5f) * pixelSize;

        float3 samplePixelPos = screenCenter
                                    + (((x - cam->width * 0.5f) * pixelSize + offsetX) * right)
                                    + (((y - cam->height * 0.5f) * pixelSize + offsetY) * up);
        float3 sampleDir = normalize(samplePixelPos - cam->position);
        ...
        float3 hero_emission = tracePath(triangles, spheres, pointLights, materials, textures, texturedata, bumpMaps, bumpMapData, normalMaps, normalMapData,bvhNodes,triIdx, triCount, sphCount, poiCount, matCount, texCount, bumpMapCount, bounces, sampleDir, orig, &seed0, &seed1, albedoOut, normalOut, id, hero_wavelength);;
        ...
        accum_emission += path_contribution;
    }

    // Divide by the number of samples to get the average
    accum_emission /= samples;
    ...
    tracePictureMaps(triangles, spheres, materials,bvhNodes,triIdx, triCount, sphCount, matCount, dir, orig, albedoOut, normalOut, id);
